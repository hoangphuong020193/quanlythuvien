...
namespace Library.Common
{
    public static class ImageProcessing
    {
        ...
        public static byte[] CreateThumbnail(byte[] sourceImage, int size)
        {
            byte[] thumbnail;
            int largestSide = (size < 0 || size > 400) ? 400 : size;
            using (MemoryStream startMemoryStream = new MemoryStream(), newMemoryStream = new MemoryStream())
            {
                ...
                if (startBitmap.Height > startBitmap.Width)
                {
                    ...
                    hwRatio = (double)largestSide / (double)startBitmap.Height;
                    newWidth = (int)(hwRatio * (double)startBitmap.Width);
                }
                else
                {
                    newWidth = largestSide;
                    hwRatio = (double)largestSide / (double)startBitmap.Width;
